...
xhkConfig * xhkInit(Display *xDisplay)
{
    xhkConfig *config;
    int i;
    config = static_cast<xhkConfig *>(malloc(sizeof(xhkConfig)));
    if (xDisplay != NULL) {
    ...
    config->hklist = static_cast<xhkHotkey *>(malloc(sizeof(xhkHotkey)));
        ...
        void *arg1, void *arg2, void *arg3)
{
    ...
    while (h->next != NULL 
        ...
        h = h->next;
    if (h->next == NULL) {
        h->next = static_cast<xhkHotkey *>(malloc(sizeof(xhkHotkey)));
        h->next->next = NULL;
    } else {
        fprintf(stderr,
